...
describe('ArcDataExport', () => {
  ...
  describe('startExport()', () => {
    ...
    it('opens success toast', async () => {
      const element = await fileFixture();
      await element.startExport();
      const toast = /** @type any */ (element.shadowRoot.querySelector('#exportComplete'));
      assert.isTrue(toast.opened);
    });

    it('opens error toast', async () => {
      const element = await basicFixture();
      try {
        await element.startExport();
      } catch (e) {
      ...
      const toast = /** @type any */ (element.shadowRoot.querySelector('#exportError'));
      assert.isTrue(toast.opened);
    });
